...
<div class="container-xxl py-5">
    <div class="container">
        <?php
        // Retrieve selected category value from URL parameter
        $selected_category = $_GET['category'];
        ?>
        <h1 class="text-center mb-5 wow fadeInUp" data-wow-delay="0.1s">Category: <?php echo $selected_category; ?></h1>
        <div class="tab-class text-center wow fadeInUp" data-wow-delay="0.3s">
            <div class="tab-content">
                <div id="tab-1" class="tab-pane fade show p-0 active">
                    <?php
                    // Query to fetch jobs based on selected category
                    $jobQuery = "SELECT v.*, c.logo FROM tbl_vacancy v
                                INNER JOIN tbl_company c ON v.company_name = c.company_name
                                WHERE v.company_category = '$selected_category'";
                    $jobResult = mysqli_query($con, $jobQuery);

                    // Check if there are any jobs found
                    if (mysqli_num_rows($jobResult) > 0) {
                        // Loop through the fetched jobs
