...
<nav class="navbar navbar-expand-lg bg-white navbar-light shadow sticky-top p-0">
        ...
        <h1 class="text-center mb-5 wow fadeInUp" data-wow-delay="0.1s">Search Result</h1>

        <?php
        include 'auth/php/config.php'; // Include your database connection file
        ...
        if (!empty($keyword)) {
            $conditions[] = "(v.job_title LIKE '%$keyword%' OR v.job_description LIKE '%$keyword%' 
            OR v.company_category LIKE '%$keyword%' OR v.location LIKE '%$keyword%'
            OR c.company_detail LIKE '%$keyword%' OR c.company_address LIKE '%$keyword%'
            OR c.company_email LIKE '%$keyword%' OR c.company_contact LIKE '%$keyword%'
            OR c.company_owner LIKE '%$keyword%')";
        }

        if (!empty($category)) {
            $conditions[] = "v.company_category = '$category'";
        }

        if (!empty($company)) {
            $conditions[] = "v.company_name = '$company'";
        }
        ...
        mysqli_close($con);
        ?>
                    <a class="d-block text-center mb-4">No more results.</a>
